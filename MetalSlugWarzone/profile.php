...
if(msw_is_post()){
    ...
    if($action==='character'){
        ...
        msw_flash('Mother Base skin is controlled by your persistent global FOB deployment and cannot be redeployed independently.','info');
    }
...
<?php if($fob):?><div class="profile-base-summary"><img src="<?=msw_e(msw_url($base['image']))?>" alt=""><div><span class="badge"><?=msw_e($base['type'])?></span><span class="badge"><?=msw_e($fobWorld?msw_fob_world_name($fobWorld):'FOB WORLD')?></span><h3><?=msw_e($base['name'])?></h3><p><?=msw_e($base['climate'])?> · <?=intval($count['hardware']??0)?> captured hardware units · persistent overview slot <?=intval($fob['slot_index'])+1?>.</p><?php if($visitRelation!==null):?><a class="btn" href="<?=msw_e(msw_url('mother_base.php?owner='.$id))?>"><?=$id===$uid?'Enter Your Mother Base':'Visit Mother Base'?></a><?php else:?><span class="badge">Friend or Strike Force access required</span><?php endif;?></div></div>
<?php else:?><div class="empty"><?=$id===$uid?'Global FOB not deployed yet. Complete the Earth overview and biome-coherent base selection to activate your permanent Mother Base.':'This commander has not deployed a global FOB yet.'?></div><?php if($id===$uid):?><div class="actions"><a class="btn" href="<?=msw_e(msw_url('fob_globe.php'))?>">Deploy Global FOB</a></div><?php endif;?><?php endif;?>
<?php msw_panel_end(); ?>
</section>
...
<?php msw_panel('Global FOB Placement','LOCKED WORLD IDENTITY');?><?php if($fob):?><p class="muted-copy">Your FOB biome, world instance, slot and Mother Base skin are persistent. They cannot be independently switched from Account Options, preventing duplicated or incoherent world presence.</p><div class="grid g3"><?php msw_stat('Biome',$biome['name']??'Unknown',$biome['climate']??'');msw_stat('World',$fobWorld?msw_fob_world_name($fobWorld):'Unknown','Shard '.intval($fobWorld['shard_index']??0));msw_stat('Slot','#'.(intval($fob['slot_index'])+1),'X '.intval($fob['x']).' · Y '.intval($fob['y']));?></div><div class="actions"><a class="btn" href="<?=msw_e(msw_url('fob_world.php'))?>">Open Overview World</a></div><?php else:?><p class="muted-copy">No global FOB has been deployed on this account.</p><a class="btn" href="<?=msw_e(msw_url('fob_globe.php'))?>">Begin Global FOB Deployment</a><?php endif;?><?php msw_panel_end(); ?>
</section><?php endif;?>
</div>
<?php msw_footer(); ?>
