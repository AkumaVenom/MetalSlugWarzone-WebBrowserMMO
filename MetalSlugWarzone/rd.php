...
if(msw_is_post()){
    ...
    if(!isset($cat[$key]))msw_flash('That R&D project is not available.','error');
    elseif(!msw_rd_recipe_unlocked($uid,$cat[$key],$levels))msw_flash('You have not unlocked this yet. Required: '.msw_requirement_label(msw_rd_recipe_requirements($cat[$key])).'.','error');
...
?>
<section class="hero"><div class="eyebrow">R&D TEAM // LEVEL <?=intval($levels['rd'])?> · MEDICAL // LEVEL <?=intval($levels['medical'])?></div><h1>RESEARCH & <span>DEVELOPMENT</span></h1><p>R&D and Medical Team levels unlock better battlefield gear. Advance R&D for stronger Fulton systems, raise Medical for combat supplies, and spend materials to manufacture equipment for your inventory.</p></section>
