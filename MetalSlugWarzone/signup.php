...
if(msw_is_post()){
    ...
    $motherBase=(string)($_POST['mother_base']??'land_dirt');
    $motherBases=msw_mother_base_catalog();
    ...
    if(!preg_match('/^[A-Za-z0-9_]{3,24}$/',$name)){
    ...
    }elseif(!isset($motherBases[$motherBase])){
        $error='Invalid Mother Base selection.';
    }else{
        $db=msw_db();
        ...
        try{
            ...
            msw_stmt('INSERT INTO users(username,password_hash,character_key,mother_base_key) VALUES(?,?,?,?)','ssss',[$name,$hash,$character,$motherBase]);
            $uid=(int)$db->insert_id;
            msw_initialize_player($uid);
            $db->commit();
            session_regenerate_id(true);
            $_SESSION['uid']=$uid;
            msw_redirect('dashboard.php');
...
<form method="post">
    ...
    <label class="signup-section-label">Mother Base / FOB</label>
    <p class="muted-copy">Choose the physical Mother Base your staff and captured hardware will occupy. You can change this later from Account Options without losing progression.</p>
    <div class="mother-base-choice-grid">
    <?php foreach(msw_mother_base_catalog() as $key=>$entry): ?>
        <label class="mother-base-choice"><input type="radio" name="mother_base" value="<?=msw_e($key)?>" <?=$key==='land_dirt'?'checked':''?>><img src="<?=msw_e(msw_url($entry['image']))?>" alt=""><span><b><?=msw_e($entry['name'])?></b><small><?=msw_e($entry['type'])?> · <?=msw_e($entry['climate'])?></small></span></label>
    <?php endforeach; ?>
    </div>
