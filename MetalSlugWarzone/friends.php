    ...
    if($action==='send'){
        ...
        if(!$target||(int)$target['id']===$uid){
            ...
            if($reciprocal){
                ...
                try{
                    ...
                    $db->commit();
                    msw_console_event_for_user($uid,'SOCIAL','FRIEND', 'Friendship established with '.(string)$target['username'].'.',['friend_id'=>$targetId,'friend'=>(string)$target['username']]);msw_flash('You both sent requests—friendship established.','success');
                ...
                try{
                    ...
                    msw_console_event_for_user($uid,'SOCIAL','FRIEND REQ','Friend request sent to '.(string)$target['username'].'.',['target_id'=>$targetId,'target'=>(string)$target['username']]);msw_flash('Friend request sent.','success');
                }catch(Throwable $e){
        ...
        if($request){
            ...
            try{
                ...
                msw_flash('Friend added.','success');
            }catch(Throwable $e){$db->rollback();throw $e;}
        }
...
?>
<div class="grid g2">
<?php msw_panel('Friends',count($friends).' COMMANDERS'); ?>
