...
if(msw_is_post()){
    ...
    try{
        ...
        if(!$row||$row['status']!=='active'||(int)$row['version']!==$version){$db->rollback();msw_flash('The battle moved on before that order landed. The latest turn has been loaded.','warning');msw_redirect('battle.php?id='.$id);}
...
<section><?php msw_panel('Combat Orders','CHOOSE YOUR ACTION'); ?>
...
<div class="alert <?=$resultKind?>" style="margin-top:12px">Battle complete: <?=msw_e(strtoupper((string)$row['status']))?>. Your operative and recovered roster return fully combat-ready after the battle.</div><div class="actions"><a class="btn" href="<?=msw_e(msw_url($continuePage))?>">Continue</a><a class="btn secondary" href="<?=msw_e(msw_url('base.php'))?>">Mother Base</a></div>
<?php endif; ?>
