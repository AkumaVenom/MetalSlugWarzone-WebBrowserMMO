...
if(msw_is_post()){
    ...
    if($action==='create'&&!$membership){
        ...
        msw_stmt('DELETE FROM strike_force_members WHERE user_id=?','i',[$uid]);msw_console_event_for_user($uid,'SOCIAL','FORCE LEAVE','Left Strike Force ['.(string)$membership['tag'].'].',['strike_force_id'=>(int)$membership['strike_force_id'],'tag'=>(string)$membership['tag']]);msw_flash('You left the Strike Force.','success');
    }
...
?>
<section class="hero"><div class="eyebrow">PLAYER ALLIANCES</div><h1>STRIKE <span>FORCES</span></h1><p>Create or join a Strike Force to fight under one banner. Members can visit one another's Mother Bases and see their recovered staff and hardware.</p></section>
