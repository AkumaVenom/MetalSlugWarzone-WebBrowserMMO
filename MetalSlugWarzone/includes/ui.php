...
require_once __DIR__ . '/audio_ui.php';
...
function msw_footer(): void {
    ?><footer class="footer"><span>METAL SLUG WARZONE // YOUR WARZONE, ALWAYS READY</span><span>v<?=msw_e((string)msw_config()['version'])?></span></footer></main><?php msw_audio_controls(); ?><script src="<?=msw_e(msw_asset_url('assets/js/msw.js'))?>"></script><?php
    if(msw_user_id()>0){
...
function msw_pvp_mode_label(string $mode): string { return ['live'=>'Live PvP','live_ai'=>'Live AI Battle','snapshot'=>'Quick AI Duel','ai_sim'=>'AI Battle'][$mode]??ucwords(str_replace('_',' ',$mode)); }
function msw_pvp_status_label(string $status): string { return ['active'=>'In Progress','player1_win'=>'Commander 1 Victory','player2_win'=>'Commander 2 Victory'][$status]??ucwords(str_replace('_',' ',$status)); }
function msw_dispatch_status_label(string $status): string { return ['pending'=>'In Progress','success'=>'Success','failed'=>'Failed','failure'=>'Failed'][$status]??ucwords(str_replace('_',' ',$status)); }
function msw_fob_result_label(string $result): string { return ['pending'=>'In Progress','attacker_win'=>'Attacker Victory','defender_win'=>'Defense Held','protected_abort'=>'Blocked by Shield'][$result]??ucwords(str_replace('_',' ',$result)); }
