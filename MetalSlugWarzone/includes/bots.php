...
function msw_bot_resolve_due_dispatches(int $uid): int {
    ...
    $due=msw_all("SELECT id FROM dispatch_missions WHERE user_id=? AND result='pending' AND finish_at<=NOW() ORDER BY id LIMIT 2",'i',[$uid]);
    $resolved=0;
    foreach($due as $row){
        $db=msw_db();$db->begin_transaction();
        try{
            $mission=msw_one("SELECT * FROM dispatch_missions WHERE id=? AND user_id=? AND result='pending' FOR UPDATE",'ii',[(int)$row['id'],$uid]);
            ...
            $ids=array_values(array_unique(array_filter(array_map('intval',json_decode((string)$mission['unit_ids_json'],true)?:[]),fn($id)=>$id>0)));
            ...
            msw_level_up_user($uid,$success?60:15);
            msw_recalculate_base($uid);
            $db->commit();
            $resolved++;
            msw_bot_set_activity($uid,$success?'Combat Unit completed '.(string)($definition['name']??$mission['mission_key']):'Combat Unit returned from '.(string)($definition['name']??$mission['mission_key']));
...
function msw_bot_fob_snapshot_locked(int $id): array {
...
function msw_bot_autonomous_fob_raid(int $attackerId): bool {
    $target=msw_one("SELECT u.id FROM bot_commanders b JOIN users u ON u.id=b.user_id WHERE b.enabled=1 AND u.id<>? AND (u.fob_protection_until IS NULL OR u.fob_protection_until<=NOW()) ORDER BY RAND() LIMIT 1",'i',[$attackerId]);
    if(!$target)return false;$defenderId=(int)$target['id'];$db=msw_db();$db->begin_transaction();
    try{
        $low=min($attackerId,$defenderId);$high=max($attackerId,$defenderId);
        $locked=msw_all('SELECT * FROM users WHERE id IN (?,?) ORDER BY id FOR UPDATE','ii',[$low,$high]);$users=[];foreach($locked as $r)$users[(int)$r['id']]=$r;
        $attacker=$users[$attackerId]??null;$defender=$users[$defenderId]??null;if(!$attacker||!$defender)throw new RuntimeException('Bot FOB target unavailable.');
        $cooldown=max(60,(int)msw_config('fob_attack_cooldown_seconds'));
        if(!empty($attacker['last_fob_attack_at'])&&strtotime((string)$attacker['last_fob_attack_at'])>time()-$cooldown){$db->rollback();return false;}
        if(!empty($defender['fob_protection_until'])&&strtotime((string)$defender['fob_protection_until'])>time()){$db->rollback();return false;}
        ...
        if(!isset($resources[$attackerId],$resources[$defenderId]))throw new RuntimeException('Bot resource ledger missing.');
        $as=msw_bot_fob_snapshot_locked($attackerId);$ds=msw_bot_fob_snapshot_locked($defenderId);
        $teamPower=(int)array_sum(array_column($as['team'],'combat'));$security=(int)($ds['security']['score']??0);
        $ar=(int)$attacker['base_power']+($teamPower*10)+random_int(0,900);$dr=(int)$defender['base_power']+($security*12)+random_int(0,900);$win=$ar>=$dr;
        $transfer=['common_metal'=>0,'minor_metal'=>0,'precious_metal'=>0,'fuel'=>0,'biological'=>0];
        if($win){foreach($transfer as $key=>$_){$take=(int)floor((int)$resources[$defenderId][$key]*0.05);$take=min($take,$key==='precious_metal'?250:1600);$transfer[$key]=$take;if($take>0){$st=msw_stmt("UPDATE player_resources SET {$key}={$key}-? WHERE user_id=? AND {$key}>=?",'iii',[$take,$defenderId,$take]);if($st->affected_rows===1)msw_stmt("UPDATE player_resources SET {$key}={$key}+? WHERE user_id=?",'ii',[$take,$attackerId]);}}
            $protection=max(300,(int)msw_config('fob_defender_protection_seconds'));$until=date('Y-m-d H:i:s',time()+$protection);msw_stmt('UPDATE users SET fob_protection_until=? WHERE id=?','si',[$until,$defenderId]);}
        msw_stmt('UPDATE users SET last_fob_attack_at=NOW() WHERE id=?','i',[$attackerId]);
        $as['resolution']=['roll'=>$ar];$ds['resolution']=['roll'=>$dr];$result=$win?'attacker_win':'defender_win';
        ...
        $db->commit();msw_bot_set_activity($attackerId,$win?'Successful autonomous FOB infiltration':'FOB infiltration repelled');return true;
    }catch(Throwable $e){$db->rollback();throw $e;}
}
...
function msw_bot_simulate_one(int $uid): void {
    ...
    if($roll<=52)msw_bot_move($uid,$user);
    elseif($roll<=76)msw_bot_field_action($uid,$user);
    elseif($roll<=85)msw_bot_manage_base($uid);
    elseif($roll<=92)msw_bot_dispatch_action($uid);
    elseif($roll<=97)msw_bot_autonomous_fob_raid($uid);
