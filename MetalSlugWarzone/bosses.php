...
?>
<section class="hero"><div class="eyebrow">HIGH-VALUE TARGETS</div><h1>BOSS <span>OPERATIONS</span></h1><p>Take on high-threat battles against signature war machines and creatures. Bosses cannot be Fulton recovered, but victory pays major resources and Commander XP.</p></section>
...
<article class="panel boss-card">
    <div class="panel-body boss-card-body">
        ...
        <div class="boss-card-copy"><span class="grade"><?=msw_e($b['threat'])?></span><h3><?=msw_e($b['name'])?></h3><p><?=msw_e($b['brief'])?></p><form method="post"><?=msw_csrf_field()?><input type="hidden" name="boss" value="<?=msw_e($key)?>"><button>Launch Boss Battle</button></form></div>
    </div>
</article>
<?php endforeach; ?>
</div>
