...
if(!$owner){http_response_code(404);echo json_encode(['error'=>'That Mother Base is unavailable.']);exit;}
...
if(!$presence || (int)$presence['base_owner_user_id']!==$ownerId || (string)$presence['base_key']!==$baseKey){
    http_response_code(409);echo json_encode(['error'=>'This Mother Base changed. Reloading the latest base.','reload'=>msw_url('mother_base.php?owner='.$ownerId)]);exit;
}
...
if($previous>0&&(($now-$previous)*1000)<$minimumMs){$elapsed=(int)floor(($now-$previous)*1000);http_response_code(429);echo json_encode(['error'=>'You are moving too quickly. Try again.','retry_ms'=>max(10,$minimumMs-$elapsed)]);exit;}
$_SESSION['last_mb_move_request_at']=$now;
$direction=(string)($_POST['direction']??'');if(!in_array($direction,['up','down','left','right'],true)){http_response_code(400);echo json_encode(['error'=>'That move is not valid.']);exit;}
