...
msw_header('FOB Strike Operations','fob.php');msw_alert(msw_flash());
?>
<section class="hero"><div class="eyebrow">FOB STAFF STRIKE OPERATIONS</div><h1>STRIKE <span>OPERATIONS</span></h1><p>FOB strike teams are separate from normal Dispatch Missions, but any staff you send are unavailable for both until they return. You can run several invasions at once, and every operation keeps progressing while you are away.</p></section>
...
<?php msw_panel('FOB Invasion Dispatches','STAFF STRIKE HISTORY');?><table><thead><tr><th>ID</th><th>Target</th><th>Team</th><th>Chance</th><th>Status</th><th>Resolution</th></tr></thead><tbody>
<?php foreach($rows as $row):$ids=json_decode((string)$row['unit_ids_json'],true)?:[];?>
<tr><td>#<?=intval($row['id'])?></td><td><a href="<?=msw_e(msw_url('fob_target.php?id='.(int)$row['defender_user_id'].'&world='.(int)$row['world_id']))?>"><?=msw_e($row['defender'])?></a></td><td><?=count($ids)?> staff</td><td><?=number_format((float)$row['success_chance']*100,1)?>%</td><td><span class="badge"><?=msw_e(msw_dispatch_status_label((string)$row['result']))?></span></td><td><?php if($row['result']==='pending'):?><span data-countdown="<?=msw_e(date(DATE_ATOM,strtotime((string)$row['finish_at'])))?>"></span><?php elseif(!empty($row['raid_id'])):?><a class="btn small secondary" href="<?=msw_e(msw_url('fob_result.php?id='.(int)$row['raid_id']))?>">Raid Report</a><?php else:?>Target was shielded on arrival<?php endif;?></td></tr>
<?php endforeach;?></tbody></table><?php if(!$rows):?><div class="empty">No FOB staff strike teams have been launched yet.</div><?php endif;?><?php msw_panel_end();?></section>
<?php msw_footer(); ?>
