...
?>
<section class="hero"><div class="eyebrow">FOB STAFF INVASION DISPATCH</div><h1>STRIKE <span>LEDGER</span></h1><p>FOB invasion dispatches are separate from standard Combat Unit Dispatch missions, but both reserve the same persistent staff rows. Strike teams may now cross shard boundaries; the ledger stores the target shard and completion still resolves exactly once from MySQL timestamps after browser or server restarts.</p></section>
<div class="actions" style="margin-top:14px"><a class="btn secondary" href="<?=msw_e(msw_url('fob_world.php'))?>">← Home Shard</a><a class="btn secondary" href="<?=msw_e(msw_url('dispatch.php'))?>">Standard Dispatch Missions</a><span class="badge"><?=$pending?> ACTIVE</span></div>
