...
if(msw_is_post()){
    ...
    if($opp<=0||$opp===$uid||!$target){msw_flash('Choose a valid PvP opponent.','error');msw_redirect('pvp.php');}
...
?>
<section class="hero"><div class="eyebrow">COMMANDER VS COMMANDER COMBAT</div><h1>LIVE <span>PvP</span></h1><p>Challenge active players in turn-based combat, take on an AI commander live, or launch a quick AI duel for an instant result. Choose your opponent and prove who controls the warzone.</p></section>
<div class="grid g2" style="margin-top:18px">
<section><?php msw_panel('Challenge Commander','PLAYERS / AI RIVALS');?><table><thead><tr><th>Commander</th><th>Grade</th><th>Power</th><th>Battle</th></tr></thead><tbody>
...
<?php if($bot):?><div class="actions"><form method="post"><?=msw_csrf_field()?><input type="hidden" name="opponent_id" value="<?=intval($p['id'])?>"><input type="hidden" name="match_mode" value="live_ai"><button class="small">Fight Live AI</button></form><form method="post"><?=msw_csrf_field()?><input type="hidden" name="opponent_id" value="<?=intval($p['id'])?>"><input type="hidden" name="match_mode" value="snapshot"><button class="small secondary">Quick AI Duel</button></form></div><?php else:?><form method="post"><?=msw_csrf_field()?><input type="hidden" name="opponent_id" value="<?=intval($p['id'])?>"><input type="hidden" name="match_mode" value="live"><button class="small">Challenge Player</button></form><?php endif;?>
</td></tr><?php endforeach;?></tbody></table><?php msw_panel_end();?></section>
<section><?php msw_panel('Recent PvP Battles','ACTIVE / RECENT');?><table><thead><tr><th>Match</th><th>Mode</th><th>Status</th><th></th></tr></thead><tbody><?php foreach($matches as $m):?><tr><td><?=msw_e($m['p1'])?><?=((int)$m['b1']===1?' [AI]':'')?> vs <?=msw_e($m['p2'])?><?=((int)$m['b2']===1?' [AI]':'')?></td><td><?=msw_e(msw_pvp_mode_label((string)$m['match_mode']))?></td><td><?=msw_e(msw_pvp_status_label((string)$m['status']))?></td><td><a class="btn small secondary" href="<?=msw_e(msw_url('pvp_match.php?id='.(int)$m['id']))?>">View</a></td></tr><?php endforeach;?></tbody></table><?php msw_panel_end();?></section>
</div><?php msw_footer(); ?>
