...
if(msw_is_post()){
    ...
    $target=msw_one('SELECT id,is_bot FROM users WHERE id=?','i',[$opp]);
    ...
    try{$id=msw_pvp_create_match($uid,$opp,$mode);msw_redirect('pvp_match.php?id='.$id);}catch(Throwable $e){msw_flash($e->getMessage(),'error');msw_redirect('pvp.php');}
}
