...
function msw_header(string $title,string $active=''): void {
    ...
    if($u && basename((string)($_SERVER['SCRIPT_NAME']??''))!=='mother_base.php') msw_mb_presence_leave((int)$u['id']);
    ?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=msw_e($title)?> · Metal Slug Warzone</title><link rel="stylesheet" href="<?=msw_e(msw_url('assets/css/msw.css'))?>"></head><body>
    <div class="scanline"></div><header class="topbar"><a class="brand" href="<?=msw_e(msw_url($u?'dashboard.php':'index.php'))?>"><span class="brand-mark">MSW</span><span><b>METAL SLUG</b><em>WARZONE // COMMAND NETWORK</em></span></a>
    <?php if($u): $c=$chars[$u['character_key']]??reset($chars);$p=msw_user_progress($u); ?><div class="commander-chip"><img src="<?=msw_e(msw_url($c['sprite']))?>" alt=""><span class="commander-chip-copy"><b><?=msw_e($u['username'])?></b><small>LV <?=intval($p['level'])?> · <?=msw_e($u['base_grade'])?> · PWR <?=number_format((int)$u['base_power'])?></small><span class="commander-xp-row"><i><em style="width:<?=round((float)$p['percent'],2)?>%"></em></i><small>XP <?=number_format((int)$p['current_xp'])?> / <?=number_format((int)$p['required_xp'])?></small></span></span></div><?php endif; ?></header>
...
function msw_stat(string $label,string|int $value,string $sub=''): void { ?><div class="stat"><small><?=msw_e($label)?></small><strong><?=msw_e($value)?></strong><?php if($sub!==''):?><em><?=msw_e($sub)?></em><?php endif;?></div><?php }
function msw_resource_strip(int $uid): void { $r=msw_resources($uid); ?><div class="resource-strip"><?php foreach(['common_metal'=>'Common Metal','minor_metal'=>'Minor Metal','precious_metal'=>'Precious','fuel'=>'Fuel','biological'=>'Bio','strategic_devices'=>'Strategic'] as $k=>$n):?><span><small><?=msw_e($n)?></small><b><?=number_format((int)($r[$k]??0))?></b></span><?php endforeach;?></div><?php }
